...
app.controller('cleaningController', ["$scope", "$rootScope", "generalService", "tookanService",
	...
	$scope.payment.callback		= function(response)
	{
		...
		generalService
			...
			.then(aResponse => {
				...
				return tookanService.create_task(aResponse.order.customer, aResponse.order.order, 'appointment', $scope.Util.tookanapp_teams.cleaning);
			})
			.then(bResponse => {
				// send details to db
